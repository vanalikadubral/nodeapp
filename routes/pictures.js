...
/* 
 * GET a list of the current Pictures DB
 */
exports.list = function(req, res) {
  res.app.get('connection').query('SELECT * FROM ebdb.PictureLogs order by logTimestamp desc', function(err, rows) {
    if (err) {
      res.send(err);
    } else {
      // console.log(JSON.stringify(rows));
      res.render('pictures', {title: 'Pictures Log', pictures: rows});
  }});
};


/*
 * POST a single picture record
 */
...
exports.fileupload = function(req, res) {
  ...
  //  INSERT into picture log DB
  req.app.get('connection').query('INSERT INTO `ebdb`.`PictureLogs` set created = now(), ?', data, function(err) {
    if (err) {
	  res.send(err);
    } else {

	  var params = {
		Bucket: 'elasticbeanstalk-us-east-1-637141817247/data/images',
		Key: logFilename,
